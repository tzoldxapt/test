...
workflow ReIndex-AllDatabases
{
	...
	ForEach ($TableName in $DatabaseQuery.Tables.Name)
	{
		...
		$TableNames = Inlinescript {
			...
			$Conn = New-Object System.Data.SqlClient.SqlConnection("Server=tcp:$using:SqlServer,$using:SqlServerPort;Database=$TableName;User ID=$using:SqlUsername;Password=$using:SqlPass;Trusted_Connection=False;Encrypt=True;Connection Timeout=30;")
...
"@              
		...
		Write-Verbose "Finished Indexing of " + $
